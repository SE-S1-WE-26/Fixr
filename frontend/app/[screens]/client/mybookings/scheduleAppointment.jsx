...
const ScheduleAppointment = () => {
    ...
    const fetchWorkerSchedules = async () => {
        try {
            const response = await axios.get(`http://192.168.8.101:8010/slots/${handymanId}`);
    ...
    const fetchJobAndWeather = async () => {
        ...
        if (handymanId) {
            try {
                const response_handymen = await axios.get(`http://192.168.8.101:8010/worker/${handymanId}`);
    ...
    const updateJob = async ({ scheduledDate, scheduledTime, scheduledWorkerId }) => {
        ...
        try {
            ...
            const response = await fetch(`http://192.168.8.101:8010/job/update/${jobId}`, {
                method: 'PUT',
                headers: {
    ...
    const handleSlotConfirm = () => {
        if (selectedSlot) {
            ...
            if (requestBody.occupiedSlots[0].from && requestBody.occupiedSlots[0].to && requestBody.occupiedSlots[0].date && requestBody.occupiedSlots[0].jobId) {
                axios.put(`http://192.168.8.101:8010/slots/update/${handymanId}`, requestBody)
                    .then(() => {
