...
const ProfileCard = () => {
  ...
  const fetchMyData = async () => {
    ...
    try {
      ...
      setWorkerData(data); // Set the retrieved data to state
    } catch (error) {
      console.error(error);
    } finally {
  ...
  useEffect(() => {
    fetchMyData(); // Call the fetch function when the component mounts
  }, []);
        ...
        <View>
            ...
            <View className="py-2 gap-1">
              <View className="flex flex-row">
                ...
                <Text className="text-sm md:text-base">{workerData.age} Years</Text>
              </View>
              <View className="flex flex-row">
                <Text className="font-medium text-sm md:text-base">Experience : </Text>
                <Text className="text-sm md:text-base">{workerData.experience} Years</Text>
              </View>
            </View>
            <View className='flex flex-row justify-between mt-2'>
              <View className='px-3 md:px-5 bg-green-900 rounded-lg'>
                <Text className="text-sm md:text-lg font-bold text-white">{workerData.phone}</Text>
              </View>
              <View className='px-3 md:px-5 bg-black-900 rounded-lg'>
                <Text className="text-sm md:text-lg font-bold text-white">{workerData.userId.location}</Text>
              </View>
            </View>
          </View>
      )}
